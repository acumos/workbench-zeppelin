...
if [ "$#" -ne 5 ]; then
    echo "Usage: ./pull_krb_from_cmlp_datasource.sh \${configure_mgt_url} \${configure_mgt_uid} \${configure_mgt_password} \${datasource_ns} \${CMLP_datasource_id}" 
    ...
    echo "      \${datasource_namespace}: CMLP datasource namespace"
    echo "      \${CMLP_datasource_id}:  CMLP datasource id"
    exit 1
fi
...
#echo "below is print out of output file"
#cat $tmp_output_filee
